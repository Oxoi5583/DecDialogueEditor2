    ...
    while (EngineWindow::Ref()->is_running()){
        ...
        */
        ...
        if(EventServer::Ref()->has<EventMouseJustClickedOnWorld>()){
            auto events = EventServer::Ref()->poll<EventMouseJustClickedOnWorld>();
            vec2 clicked_pos;
            for(auto event : events){
                clicked_pos = event.click_pos;
            }

            if(test_rect.is_point_inside(clicked_pos)){
        ...
        vec4 color = (test_rect.is_point_inside(mouse_pos)) ? vec4(1.0f,0.0f,0.0f,1.0f) : vec4(0.0f,1.0f,0.0f,1.0f);
